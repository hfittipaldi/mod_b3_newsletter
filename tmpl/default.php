...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" role="form" novalidate>
    <fieldset><legend class="modnsintro"><?php echo $pre_text; ?></legend>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="m_name<?php echo $unique_id; ?>" size="<?php echo $nameWidth; ?>"<?php echo (($errors & 1) != 1) ? ' value="' . htmlentities($subscriberName, ENT_COMPAT, "UTF-8") . '"' : ''; ?> placeholder="Nome completo" />
        </div>
        <div class="form-group">
            <label for="m_email<?php echo $unique_id; ?>" class="sr-only"><?php echo $myEmailLabel; ?></label>
            <input type="email" class="form-control" name="m_email<?php echo $unique_id; ?>" size="<?php echo $emailWidth; ?>"<?php echo (($errors & 2) != 2) ? ' value="' . htmlentities($senderEmail, ENT_COMPAT, "UTF-8") . '"' : ''; ?> placeholder="Email" />
        </div>
...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" role="form">
    <fieldset><legend class="modnsintro"><?php echo $pre_text; ?></legend>
        ...
        <div class="form-group">
            <label for="m_name<?php echo $unique_id; ?>" class="sr-only"><?php echo $myNameLabel; ?></label>
            <input type="text" class="form-control" name="m_name<?php echo $unique_id; ?>" size="50"<?php echo (($errors & 1) != 1) ? ' value="' . $session->get('postName', '') . '"' : ''; ?> placeholder="Nome completo" />
        </div>
        <div class="form-group">
            <label for="m_email<?php echo $unique_id; ?>" class="sr-only"><?php echo $myEmailLabel; ?></label>
            <input type="email" class="form-control" name="m_email<?php echo $unique_id; ?>" size="50"<?php echo (($errors & 2) != 2) ? ' value="' . $session->get('postEmail', '') . '"' : ''; ?> placeholder="Email" />
        </div>
